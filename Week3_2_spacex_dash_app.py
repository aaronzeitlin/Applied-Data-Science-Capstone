...
def get_pie_chart(entered_site):
    ...
    if entered_site == "ALL":
        ...
        # return the outcomes piechart for a selected site using the binary 'class' column
        fig = px.pie(
...
def get_scatter_chart(entered_site, payload_range):
    ...
    if entered_site == "ALL":
        filtered_mass = spacex_df[
            (spacex_df["Payload Mass (kg)"] >= min_payload)
            & (spacex_df["Payload Mass (kg)"] <= max_payload)
        ]
        title = "Total Successful Launches by all Launch Sites"
    else:
        filtered_mass = spacex_df[
            (spacex_df["Payload Mass (kg)"] >= min_payload)
            & (spacex_df["Payload Mass (kg)"] <= max_payload)
